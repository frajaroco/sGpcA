...
## error checking
  ...
  ## error checking
  if(inherits(X,"matrix") != TRUE && inherits(X,"dgCMatrix") != TRUE){
    stop("X must be a matrix of class 'matrix'  or 'dgCMatrix' ")
  }

  if(inherits(Q,"matrix") != TRUE && inherits(Q,"dgCMatrix") != TRUE){
    stop("Q must be a matrix of class 'matrix'  or 'dgCMatrix' ")
  }


  if(inherits(R,"matrix") != TRUE && inherits(R,"dgCMatrix") != TRUE){
